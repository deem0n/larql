...
| **D-METAL-PLE** | Implement Per-Layer Embeddings in the Metal decode path | Brings Gemma 4 E2B / future-PLE-models from CPU fallback (~1670 ms/tok) onto Metal (target: ~10-20 ms/tok at E2B's compute scale = **80-150× speedup for E2B**). | **Open.** PLE math (`larql-inference/src/forward/ple.rs`): per layer, compute `gate = gelu_tanh(h × W_input_gate.T)` → `gated = gate * per_layer_input` → `contribution = gated × W_projection.T` → `RMSNorm(contribution)` → `h += normed`. 5 ops per layer × 35 layers = 175 extra dispatches per token. Most kernels already exist (matvec, geglu element-wise, rms_norm, residual_inject::add); the work is **plumbing + per-layer dispatch** + caching the precomputed per-layer-input streams in Metal buffers. Estimated 1-2 days. | New per-layer-embed dispatch in `metal/decode/encode_*.rs`; new shader `metal/shaders/per_layer_embed.rs` (or use existing matvec + geglu + residual_inject). Per-layer-input precompute in `metal/ops/full_pipeline/dispatch.rs` at prefill setup time. Gated on `LARQL_METAL_PLE=1` until parity-tested against the CPU path. |
