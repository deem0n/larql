...
| **D-METAL-KV-SHARED** | Implement cross-layer KV cache sharing in the Metal decode path | E2B's last 20 of 35 layers reuse K/V from earlier "source" layers (last non-shared sliding / global layer of the same type). Without this, layers 15-34 compute their own (wrong) K/V → attention is broken from L15 → garbage output. Same mechanism likely needed for any future KV-shared model. | **IDENTIFIED 2026-05-10 while end-to-end testing D-METAL-PLE.** E2B `index.json` has `num_kv_shared_layers: 20`. CPU implements this in `forward/layer.rs:161-169` via `arch.kv_shared_source_layer(layer)` — when `Some(src)`, the layer reads K/V from `kv_cache[src]` instead of computing fresh. **Metal has zero references to `kv_shared_source_layer`** (`grep -r kv_shared crates/larql-compute/src` is empty). Empirical confirmation: with PLE wired correctly (kernel parity ✓, routing ✓, all 35 layers fire), `LARQL_METAL_PLE=1 larql run gemma4-e2b-q4k.vindex …` still emits multilingual gibberish ("crisparametricbreakers impost billetsmixMom你就 …"), matching the symptom of broken attention from L15 onward. Compare: 31B and 4B don't have KV sharing and run fine. | New work: per-layer KV-cache routing in `metal/ops/kv_cache.rs` (or wherever the cache views live) — when `arch.kv_shared_source_layer(l)` is `Some(src)`, the attention dispatch for layer `l` should bind layer `src`'s K/V buffer instead of layer `l`'s. Also: skip the K/V projection dispatch (encode_qkv) for shared layers. Plumb `kv_shared_source` through `FullPipelineLayer` (similar to PLE plumbing) so the dispatch site can read it without re-reading the arch trait per layer. Estimated 1-2 days; mostly plumbing, no new shaders required. |
